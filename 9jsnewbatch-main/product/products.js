...
function display(d)
{
  ...
  let mydisplay = d.map(function(value,index) {
...
let unique = products.reduce(function(pre,value){
...
let newbuttons = unique.map(function(value){

  return `<button class="company-btn">${value}</button> `
})
document.getElementById('btns').innerHTML=newbuttons.join("");
